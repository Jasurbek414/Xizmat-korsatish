...
const MijozBuyurtmaForm = ({ onAddOrder, preSelectedClient }) => {
  ...
  // Sync client details when preSelectedClient changes (e.g. caller incoming or log selected)
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    // Automatically save client to DB if not exists
    const clients = getDbItem('dispatcher_clients') || [];
    const exists = clients.some(c => c.phone.replace(/\s+/g, '') === phoneVal.replace(/\s+/g, ''));
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-[#111522] border border-white/[0.04] rounded-2xl p-5 w-full flex flex-col gap-4 text-xs font-semibold text-slate-200 shadow-2xl">
      
      {/* SECTION 1: MIJOZ */}
      <div className="space-y-3">
        <div className="flex items-center gap-2 border-b border-white/[0.04] pb-2 shrink-0 select-none">
          <User className="w-4 h-4 text-slate-400" />
          <span className="font-extrabold text-[10.5px] uppercase tracking-wider text-slate-350 font-outfit">Mijoz</span>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 border-b border-white/[0.04] pb-2 shrink-0 select-none">
          ...
          <span className="font-extrabold text-[10.5px] uppercase tracking-wider text-slate-350 font-outfit">Buyurtma</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="relative">
            ...
            <select
              ...
              className="w-full bg-[#0a0d16] border border-white/[0.06] focus:border-indigo-500/50 rounded-xl pl-8 pr-2 py-2 text-xs text-slate-100 font-bold focus:outline-none transition cursor-pointer appearance-none"
            >
              {SERVICES.map(s => (
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Hajmi"
              className="w-full bg-[#0a0d16] border border-white/[0.06] focus:border-indigo-500/50 rounded-xl pl-8 pr-2 py-2 text-xs text-slate-100 font-mono font-bold focus:outline-none transition"
              required
            />
          </div>

          <div className="relative">
            <Coins className="absolute left-2.5 top-2.5 w-3.5 h-3.5 text-slate-500" />
            <input
              type="number"
              value={price}
              onChange={(e) => setPrice(e.target.value)}
              placeholder="Narx"
              className="w-full bg-[#0a0d16] border border-white/[0.06] focus:border-indigo-500/50 rounded-xl pl-8 pr-2 py-2 text-xs text-slate-100 font-mono font-bold text-indigo-400 focus:outline-none transition"
              required
            />
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <select
            ...
            className="w-full bg-[#0a0d16] border border-white/[0.06] focus:border-indigo-500/50 rounded-xl pl-9.5 pr-3 py-2 text-xs text-slate-100 font-bold focus:outline-none transition cursor-pointer appearance-none"
          >
            {CAMPAIGNS.map(c => (
      ...
      <div className="flex gap-4 pt-2 shrink-0 select-none">
        <button
          ...
          className="flex-[1.2] py-2.5 bg-[#1c243b] hover:bg-[#232d4a] text-slate-350 hover:text-slate-200 font-extrabold rounded-xl transition cursor-pointer text-[10px] uppercase tracking-wider font-outfit flex items-center justify-center gap-1.5 border border-white/[0.04]"
        >
          <Plus className="w-3.5 h-3.5 text-indigo-400" /> Mijoz saqlash
        </button>
        <button
          type="submit"
          className="flex-1 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white font-extrabold rounded-xl transition cursor-pointer text-[10px] uppercase tracking-wider shadow-lg shadow-indigo-500/10 active:scale-95 font-outfit flex items-center justify-center gap-1.5"
        >
          <CheckCircle className="w-3.5 h-3.5 fill-white text-indigo-600" /> Buyurtma
        </button>
      </div>
